...
`supplyAsync` — создат новый асинхроннй поток (In: '-'  Out: '+');
метод `get` — это **синхронный метод** который иннициирует всю работу;
![](5.e5QW3v.jpg)

`thenAccept` — позволяет навешать сколько угодно **асинхронных callbacks-методов** на момент завершения выполнения (In: '+'  Out: '-');
![](6.X9xovf.jpg)
 
`thenApply` — **трансформация**: добавление нескольких callbacks-методов (In: '+'  Out: '+');
![](8.kcPi0L.jpg)

`thenCompose` — **композиция**: сценарий каких-то последовательных действий (In: '+'  Out: '+');
![](9.df15zW.jpg)

`thenCombine` — **комбинирование**: выполнить два действия одновременно И только после того как будет доступно оба результат выполнить функцию... (In: '+'  Out: '+');
![](10.TX34uC.jpg)

Распаралеливание асинхронных процессов для ускорения вычислений (`...Async`)
![](11.aIDLRW.jpg)
![](12.jpkHmS.jpg)

`AnyOf` — нахождение первого освободившегося потока чтобы передать ему очередную задачу (In: '-'  Out: '-')
![](13.5uMJhr.jpg)

`applyToEither` — нахождение первого освободившегося потока чтобы передать ему очередную задачу И выполнить еще что-то (In: '+'  Out: '+')
![](14.Zy5TbM.jpg)
