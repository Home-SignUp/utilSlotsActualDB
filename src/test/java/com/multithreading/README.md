...
`supplyAsync` — создат новый асинхроннй поток; `thenAccept` — позволяет навешать сколько угодно асинхронных callbacks-методов на момент завершения выполнения; метод `get` — это синхронный метод который иннициирует всю работу;
![](6.X9xovf.jpg)
