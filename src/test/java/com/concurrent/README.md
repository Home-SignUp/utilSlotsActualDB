...
---
    ...
    awaitTermination(..) — ждет завершения запущенных задач в течение какогото времени и потом останавливает исполнитель
...
ограничивает количество потоков, которые одновременно работают над одним и тем же ресурсом:
    ...
    `tryAcquire()` — проверяет если пул (поток) свободный, возвращает TRUE/FALSE
    `release()` — освобождает пул (поток)
