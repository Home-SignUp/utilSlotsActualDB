...
//        B.staticFunc(3); // статический метод создается от класса (компилятор подсказывает нормально)
        A a = new A();
        a.staticFunc(2); // статический метод тоже создается от класса (но компилятор не подсказывает) // A.2
        B b = new B();
        b.staticFunc(4); // статический метод тоже создается от класса (но компилятор не подсказывает) // B.4
        A x = new B();
        x.staticFunc(5); // A.5
    }
